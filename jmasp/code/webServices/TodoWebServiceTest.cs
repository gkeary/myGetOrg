...
public void Should_Get_List_Of_Todos_As_XML()
{
  ...
  var responseWithXML =
    WebServiceUtil.SendWebRequest("http://localhost:1901/Todo/", 
    string.Empty, "text/xml", authenticateResponse.Cookies[0]); 

  var xmlReader = XmlReader.Create(
    responseWithXML.GetResponseStream());
  
  XDocument todoDoc = XDocument.Load(xmlReader);

  Assert.AreEqual("ArrayOfTodo", todoDoc.Root.Name.ToString());
  Assert.AreEqual("Todo", 
    todoDoc.Root.Elements("Todo").First().Name.ToString());
  Assert.AreEqual("Title", todoDoc.Root.Elements("Todo").
    Elements("Title").First().Name.ToString());
  Assert.AreEqual("Id", todoDoc.Root.Elements("Todo").
    Elements("Id").First().Name.ToString());
}
[Test]
public void Should_Send_Xml_To_Create_Todo()
{
  ...
  var xmlRequest = 
    WebServiceUtil.SendWebRequest(
    "http://localhost:1901/Todo/CreateWithXml", 
    xml, "text/xml", authRequest.Cookies[0]);
  
    var xmlReader = 
      XmlReader.Create(xmlRequest.GetResponseStream());
    XDocument todoDoc = XDocument.Load(xmlReader);

  Assert.AreEqual("Todo", todoDoc.Root.Name.ToString());
  Assert.AreEqual("foo" , 
    todoDoc.Root.Element(XName.Get("Title" )).Value);
}
